...
### 2D — 商務頁 ×4 ✅ 完成（2026-06-02，build 綠燈）
- 產出：`lib/products.ts`、`components/{Shop,Product,Checkout}.tsx`(use client)、`app/[locale]/{shop,product,checkout}/page.tsx`。`CartDrawer` 的 `QtyStep` 改具名匯出給 Checkout 重用。購物車吃 2A 基建（`Cart.add/setQty/remove/clear`、`useCart`）。
- ✅ 已驗：build 綠燈（9 路由×3 語系新增，共 31 頁靜態匯出）；shop 篩選/加購、product gallery/qty 帶入、checkout 明細/運費門檻/下單清空都到位。

**（原始任務說明，保留參考）先讀**：`../design-extract-auvita/ui_kits/auvita/{shop,product,checkout}.jsx`（+ `shop.html`/`product.html`/`checkout.html` 看 active prop 與組合）。
...
- **Stage 3** ✅ ogl 金箔粒子：`GoldDust.tsx` use client 薄殼 + `next/dynamic(import('./GoldDustCanvas'),{ssr:false})`；`GoldDustCanvas.tsx` 真 ogl（`import from 'ogl'` 取代 window.ogl）。裝 `ogl ^1.0.11`。Hero 調校：count 280 / baseSize 120 / **pixelRatio 2**（=超取樣倍率，>2 純燒 GPU）/ shader 透明度 ×1.0。
- **Stage 4** ✅ 上線 https://www.auvita.tw（2026-06-02）。自有 repo `github.com/GNTadmin001/Auvita`（main）；`deploy-on-push.yml`（npm@latest→npm ci→lint→build→FTP）；Secrets 全設。**CI 踩雷**：① 需 `npm install -g npm@latest`（lock 由新版 npm 生成）② lock 須同步 ③ **`FTP_USER` 寫完整 `帳號@auvita.tw`** 否則 530。首次 clean-slate、之後改增量。
